...
void* _configureADCLowSide(const void* driver_params, const int pinA,const int pinB,const int pinC){
  ...
  if(t->on_full != nullptr){
    SIMPLEFOC_ESP32_CS_DEBUG("ERROR: Low side callback is already set. Cannot set it again for timer: "+std::to_string(t->timer_id)+", group: "+std::to_string(t->group->group_id));
  ...
  for (int i = 0; i < 3; i++){
    if(_isset(adc_pins[i])){
      if(!adcInit(adc_pins[i])){
       SIMPLEFOC_ESP32_CS_DEBUG("ERROR: Failed to initialise ADC pin: "+std::to_string(adc_pins[i]) + ", maybe not an ADC pin?");
        return SIMPLEFOC_CURRENT_SENSE_INIT_FAILED;
      }
...
void* _driverSyncLowSide(void* driver_params, void* cs_params){
  ...
  SIMPLEFOC_ESP32_CS_DEBUG("Timer "+std::to_string(t->timer_id)+" enable interrupt callback.");
  ...
  SIMPLEFOC_ESP32_CS_DEBUG("Timer "+std::to_string(t->timer_id)+" enable interrupts.");
